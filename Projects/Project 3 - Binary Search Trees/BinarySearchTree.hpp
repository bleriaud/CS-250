...
private:
    ...
    */
    Node<TK, TD>* FindParentOfNode( const TK& key )
    {
		...
		while (true)
		{
			...
			else if (ptrCurrent->ptrLeft != nullptr && ptrCurrent->ptrLeft->key == key)
			{
				return ptrCurrent;
			}
			else if (ptrCurrent->ptrRight != nullptr && ptrCurrent->ptrRight->key == key)
			{
				return ptrCurrent;
			}
    ...
    */
    int GetHeight( Node<TK, TD>* ptrCurrent )
    {
		...
		{
			return GetHeight(ptrCurrent->ptrLeft) +1;
		...
		if (ptrCurrent->ptrLeft == nullptr && ptrCurrent->ptrRight != nullptr)
		{
			return GetHeight(ptrCurrent->ptrRight) + 1;

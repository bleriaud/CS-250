...
void AirTravelSimulator::Run()
{
	...
	cout << endl << "What the max capacity for the airplane?" << endl;

	int choice;
	cin >> choice;
	m_airport.SetMaxCapacity(choice);
...
bool AirTravelSimulator::LineUp()
{
	...
	int r = rand() % 1000;
	Traveller* traveller = m_pplManager.GetTraveller(r);
	m_airport.LineUp(traveller);
	m_ptrTravellers.PushFront(traveller);
